...
If users have newly assembled genomes that are not published but are desired to use as the references, these genomes sequences should be placed in `synmetap/input/user_genome`. The accompanied gene annotation files should be placed in the same folder.
...
We include this table (`synmetap/input/Taxontable_demo`) in the pipeline. It is distributed with 
