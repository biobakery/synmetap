...
#added my intention here

#modified by rby
#move the below four lines to __main__
"""
LOG_FILENAME='../rd.log'
...
def getRef(refFile):
    ...
    i=f.readline()
    #added by rby, check empty. Can only check the empty file that contains nothing. Might need to be more sophisticated.
    if i == '':
	sys.exit('Reference file '+refFile+' is empty. Please check the raw genome files.')
    ...
    while i:
	    ...
	    # Comp() is used to complement a sequene. No reverse.
...
# This is the place where the weight for each bug generating.
def getMet(directory,genDir, abund):
    ...
    for k in key:
        ...
        val=val/sum
        ab[k]=val
    #modified here by rby
    #delete './' in front of the following two equations, so can be any kind of path
    path1=directory
    path2=genDir
    for k in key:
...
def main(argv):
#added by rby
#try to add a new flag to indicate the path for log file.
    dirlog=''
